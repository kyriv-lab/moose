...
/**
 * Base class for user objects that load a structured-data file into a JSON tree and make its
 * content available to other objects. The data is always stored as a JSON tree, so derived classes
 * only need to implement read() to populate that tree from their particular file format (e.g. JSON
 * directly, or YAML converted to JSON).
 */
class JSONFileReaderBase : public GeneralUserObject
{
public:
  static InputParameters validParams();

  JSONFileReaderBase(const InputParameters & parameters);
  ...
  void getValue(const std::string & value_name, T & value) const
  {
    if (!_root.contains(value_name))
      mooseError("Attempted to get '",
                 value_name,
                 "' but the file does not contain this key directly at the root level");
  ...
  {
    if (!value_keys.size())
      mooseError("There should be at least one key to retrieve a value from the file");
  ...
  void getVector(const std::string & vector_name, std::vector<T> & vector_to_fill) const
  {
    if (!_root.contains(vector_name))
      mooseError("Attempted to get '",
                 vector_name,
                 "' but the file does not contain this key at the root level");
...
protected:
  /**
   * Read the file and load its content into _root
   * @param filename the name of the file
   */
  virtual void read(const FileName & filename) = 0;

  /// Database filename
  const FileName & _filename;
  /// File data, stored as a JSON tree
  nlohmann::json _root;
};
